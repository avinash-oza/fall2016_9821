...
std::tuple<double, double, double, double, double> binomialTree(double S, double K, double T, double q, double r, int N, double sigma)
...
// N is the number of time steps
{
    ...
    for (int i = 0 ; i < N; i++)
    {
        double intrinsicValue = std::max(K - S* std::pow(u, N - i - 1) * std::pow(d, i) , 0.0);
...
double binomialBlackScholeswithRichardsonExtrapolation(double S, double K, double T, double q, double r, int N, double sigma)
{
    ...
    return 2.0*(std::get<0>(bbs1)) - std::get<0>(bbshalfN);
}

double calculateTreesForN(int N)
{
    double S = 41.0;
    double K = 40.0;
    double T = 1.0;
...
//              << N*N*absDiff
//              << std::endl;
    return binomialBlackScholesPrice;
